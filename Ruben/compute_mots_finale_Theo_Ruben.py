...
df = avis_Id_Text_Preprocessed_joined.get_dataframe()
...
def vect_theme_tfidf(database,maxf=100000,nrange=(1,2),colonne='docs'):
    '''
    ...
    for t in database.keys():
        train[t]=vect.fit_transform(database[t][colonne])
        vect_theme[t]=vect
...
def W2VKMEANS_mots(database,fenetre=15,minimum=1,d=300,len_min=2,racine=3,num_clust=3,colonne='docs'):
    import gensim
    import numpy as np
    ...
    for t in database.keys():
        #print(t)
        sentences = np.array([str(c).split() for c in list(database[t][colonne])],dtype=object)
        ...
        vocab_theme_wv=np.array([W2V.wv.most_similar(z)[i][0] for z in vocab_theme for i in range(10)]).flatten()
        ...
        if len(vocab_theme_wv)!=0:
            vector=pd.concat([pd.DataFrame(W2V.wv[v]).T for v in vocab_theme_wv])
            vect_moyen=np.array([W2V.wv[v].T for v in vocab_theme]).mean(axis=0)
            ...
            mots_themes[t]=[vector.index[vector.label==np.bincount(groupe_theme).argmax()],
                            np.array([cos_sim(vect_moyen,W2V.wv[v]) for v in vector.index[vector.label==np.bincount(groupe_theme).argmax()]])]
        else:
...
features,vect_theme=vect_theme_tfidf(database,colonne='text_preprocessed')
...
mots_themes_WKMeans=W2VKMEANS_mots(database,colonne='text_preprocessed')
...
base_mots_theo_df =base_finale

# Write recipe outputs
base_mots_theo = dataiku.Dataset("base_mots_theo")
base_mots_theo.write_with_schema(base_mots_theo_df)

# -------------------------------------------------------------------------------- NOTEBOOK-CELL: CODE
#base_finale

# -------------------------------------------------------------------------------- NOTEBOOK-CELL: CODE
#base_finale[base_finale.methode=='WKmeans'].proximite.values[0]

#mots_finale_Theo_Ruben_df = avis_Id_Text_Preprocessed_joined_df # For this sample code, simply copy input to output


# Write recipe outputs
mots_finale_Theo_Ruben = dataiku.Dataset("mots_finale_Theo_Ruben")
mots_finale_Theo_Ruben.write_with_schema(mots_finale_Theo_Ruben_df)