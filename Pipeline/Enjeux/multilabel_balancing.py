...
def create_multilabel_dataset(n_sample=1000):
    ''' 
    Create a unevenly distributed sample data set multilabel  
    classification using make_classification function
    ...
    y: pandas.DataFrame, target vector dataframe with 5 labels
    '''
    X, y = make_multilabel_classification(n_classes=5,
                           n_features=10,n_samples=1000, random_state=10)

    y = pd.DataFrame(y, columns=['class_'+str(k) for k in range(5)])
    return pd.DataFrame(X), y
